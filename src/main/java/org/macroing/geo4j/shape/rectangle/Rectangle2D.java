...
import org.macroing.geo4j.shape.Shape2D;
import org.macroing.geo4j.shape.circle.Circle2D;
import org.macroing.geo4j.shape.ls.LineSegment2D;
...
public final class Rectangle2D implements Shape2D {
	...
	public static Rectangle2D fromPoints(final Point2D... points) {
		...
		Point2D max = Point2D.MIN;
		Point2D min = Point2D.MAX;
		
		for(final Point2D point : points) {
			max = Point2D.max(max, point);
			min = Point2D.min(min, point);
		}
		
		return new Rectangle2D(max, min);
	}
	...
	public static Rectangle2D rotate(final Rectangle2D rectangle, final AngleD angle) {
		final Point2D a = Point2D.rotateCounterclockwise(rectangle.a, angle.getRadians(), true);
		final Point2D b = Point2D.rotateCounterclockwise(rectangle.b, angle.getRadians(), true);
		final Point2D c = Point2D.rotateCounterclockwise(rectangle.c, angle.getRadians(), true);
		final Point2D d = Point2D.rotateCounterclockwise(rectangle.d, angle.getRadians(), true);
	...
	public static Rectangle2D union(final Rectangle2D a, final Rectangle2D b) {
		final Point2D min = Point2D.min(Point2D.min(a.getA(), a.getB(), a.getC(), a.getD()), Point2D.min(b.getA(), b.getB(), b.getC(), b.getD()));
		final Point2D max = Point2D.max(Point2D.max(a.getA(), a.getB(), a.getC(), a.getD()), Point2D.max(b.getA(), b.getB(), b.getC(), b.getD()));
