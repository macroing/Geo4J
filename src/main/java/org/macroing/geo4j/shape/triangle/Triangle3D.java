...
import org.macroing.geo4j.shape.SurfaceSample3D;
...
//	TODO: Add Unit Tests!
	...
	public BoundingVolume3D getBoundingVolume() {
		final Point3D a = Point3D.min(new Point3D(this.a.getPosition()), new Point3D(this.b.getPosition()), new Point3D(this.c.getPosition()));
		final Point3D b = Point3D.max(new Point3D(this.a.getPosition()), new Point3D(this.b.getPosition()), new Point3D(this.c.getPosition()));
...
//	TODO: Add Unit Tests!
	...
	public Optional<SurfaceSample3D> sample(final Point2D sample) {
		...
		final Point3D barycentricCoordinates = Point3D.sampleTriangleUniformDistribution(sample);
...
//		TODO: Add Unit Tests!
		public static Vertex3D transform(final Vertex3D vertex, final Matrix44D matrix, final Matrix44D matrixInverse) {
			...
			final Point4D position = matrix.transform(vertex.position);
			
			final Vector3D normal = Vector3D.normalize(matrixInverse.transformTranspose(vertex.normal));
...
//		TODO: Add Unit Tests!
		public static Vertex3D transformAndDivide(final Vertex3D vertex, final Matrix44D matrix, final Matrix44D matrixInverse) {
			...
			final Point4D position = matrix.transformAndDivide(vertex.position);
...
//		TODO: Add Unit Tests!
	...
	private Point2D doCreateTextureCoordinates(final Point3D pBC) {
		final Point2D pTCA = this.a.getTextureCoordinates();
		final Point2D pTCB = this.b.getTextureCoordinates();
		final Point2D pTCC = this.c.getTextureCoordinates();
