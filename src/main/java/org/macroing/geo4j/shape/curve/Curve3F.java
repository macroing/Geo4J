...
//	TODO: Add Unit Tests!
	...
	public Optional<SurfaceIntersection3F> intersection(final Ray3F ray, final float tMinimum, final float tMaximum) {
		...
		final Point3F pointI = objectToRay.transform(pointE);
		final Point3F pointJ = objectToRay.transform(pointF);
		final Point3F pointK = objectToRay.transform(pointG);
		final Point3F pointL = objectToRay.transform(pointH);
		...
		final int depth = Ints.saturate(doLog2(1.41421356237F * 6.0F * l03 / (8.0F * (Floats.max(widthA, widthB) * 0.05F))) / 2, 0, 10);
	...
	private static OrthonormalBasis33F doComputeOrthonormalBasisG(final Data data, final Matrix44F objectToRay, final Matrix44F rayToObject, final float hitWidth, final float u, final float v) {
		switch(data.getType()) {
			case CYLINDER: {
				...
				final Vector3F directionUPlane = Vector3F.normalize(objectToRay.transform(directionU));
				final Vector3F directionVPlane = Vector3F.normalize(Matrix44F.rotate(AngleF.degrees(-Floats.lerp(-90.0F, 90.0F, v), -90.0F, 90.0F), directionUPlane).transform(Vector3F.multiply(Vector3F.normalize(new Vector3F(-directionUPlane.y, directionUPlane.x, 0.0F)), hitWidth)));
				final Vector3F directionV = Vector3F.normalize(rayToObject.transform(directionVPlane));
