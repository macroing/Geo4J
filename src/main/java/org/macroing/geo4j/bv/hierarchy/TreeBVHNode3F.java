 ...
 */
package org.macroing.geo4j.bv.hierarchy;
...
import org.macroing.geo4j.shape.SurfaceIntersector3F;
...
//	TODO: Add Unit Tests!
	...
	protected float intersectionT(final Ray3F ray, final float[] tBounds) {
		return getBoundingVolume().contains(ray.getOrigin()) || getBoundingVolume().intersects(ray, tBounds[0], tBounds[1]) ? Floats.minOrDefault(this.bVHNodeL.intersectionT(ray, tBounds), this.bVHNodeR.intersectionT(ray, tBounds), Float.NaN) : Float.NaN;
	}
}